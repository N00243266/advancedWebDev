...
<x-app-layout>                   
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('All Artworks') }}                                            <!-- header for artworks index page -->
        </h2>
        @if(session('success'))
            <x-alert-success>
                {{ session('success') }}
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="font-semibold text-lg mb-4">List of all Artworks:</h3>         <!-- subtitle for artworks index page -->

                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($artworks as $artwork)
                        <div class="border p-4 rounded-lg shadow-md">
                            <a href="{{ route('artworks.show', $artwork) }}">               <!-- Link to artwork details page -->
                                <x-artwork-card 
                                    :title="$artwork->title"
                                    :genre="$artwork->genre"
                                    :image="$artwork->image"
